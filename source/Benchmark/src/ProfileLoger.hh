...
{
public:
    static void WriteBenchMark(const Time_Data& data)
    {
        FileHandle::Singelton().WriteInfo(data);
    }
...
private:
    ...
    void WriteInfo(const Time_Data& data)
    {
...
public:
    explicit Timer(const std::string& name)
    {
        m_data.m_name      = name;
        m_data.m_sec       = 0;
        m_data.m_startTime = m_timeManager.Start_Time();
        m_data.threadID    = std::hash<std::thread::id>{}(std::this_thread::get_id());
    }
    ~Timer()
    {
        m_timeManager.End_Time();
        m_data.m_sec = m_timeManager.GetTimeSec();
        Write_File();
    }
